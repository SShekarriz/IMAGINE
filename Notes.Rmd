...
colon diagram in the background. B: a taxa bar chart at the Family level. 
Consider giving mean and individual. If mean, surround with at least four 
diverse samples to show the range.
...
The file [Setup.R](./Setup.R) contains code that imports and cleans the data,
produces data frames for plotting taxa bar charts and PCoA charts, and generates
the background PCoA plot, as well as the mean and four "range" bar charts for
use with patchwork/cowplot.
...
ps_gen_df <- make_phy_df(ps_i_rel, rank = "Genus", cutoff = 0.001, 
            count = FALSE)
plt_bar = plot_tax_bar(ps_gen_df, 'Family', sample = 'Study_ID')
plt_bar
```
...
ps_wc = (ps_gen_df
         ...
         %>% filter(Genus != 'Other')
         %>% mutate(Genus = order_levs(Genus, Abundance),
                    Genus = factor(Genus, levels = rev(levels(Genus))))
         %>% select(Genus, Abundance, Family))

ps_wc = arrange(ps_wc, desc(Abundance))
set.seed(42)
wcplt = ggplot(ps_wc, aes(label = Genus, size = Abundance*1000, colour = Family)) +
    geom_text_wordcloud() +
